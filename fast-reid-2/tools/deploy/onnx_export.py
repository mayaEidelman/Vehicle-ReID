...
def get_parser():
    ...
    parser.add_argument(
        "--output", default="onnx_model", help="path to save converted onnx model"
    )
    parser.add_argument(
        "--batch-size",
        default=1,
...
if __name__ == "__main__":
    ...
    print("#" * 100)
    cfg.MODEL.WEIGHTS = "/home/labs/training/class46/Aerial-IR-REID/external_gits/fast-reid/logs/VeRi-UAV-DownSample-100/model_best.pth"
    ...
    inputs = torch.randn(
        args.batch_size, 3, cfg.INPUT.SIZE_TEST[0], cfg.INPUT.SIZE_TEST[1]
    ).to(model.device)
    model_simp = export_onnx_model(model, inputs)
